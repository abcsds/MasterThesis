...
The maximum correlation is 0.251, between the emotion `Trust` and dimension 121 of the model. Although this is double the correlation that could be found thorough chance, it does not satisfy the baseline % TODO: (wartena) Baseline ist that it needs to be 3 times the baseline? I woul think that correlation coefficients can be interpreted as absolute numbers. I woud say 0.251 is a weak correllation
...
\end{figure}
% TODO: (wartena) Did you always take the word embeddings from Bert computed for the whole sentence?
